...
if (isset($_POST['save'])) { // if save button on the form is clicked
 ...
 if ($_FILES['myfile']['size'] > 10000000) { // file shouldn't be larger than 1Megabyte
        echo "File too large!";
    } else {
        // move the uploaded (temporary) file to the specified destination
        if (move_uploaded_file($file, $destination)) {
            $sql = "INSERT INTO tpaidat (nimi, hinta, kuva, tuotetiedot) VALUES ('$_POST[tpaidat]' , '$_POST[hinta]', '$filename', '$_POST[tuotetiedot]')";
...
if (isset($_GET['file_id'])) {
    ...
    $sql = "SELECT * FROM tpaidat WHERE id=$id";
    $result = mysqli_query($conn, $sql);
    ...
    if (file_exists($filepath)) {
        ...
        $updateQuery = "UPDATE tpaidat SET downloads=$newCount WHERE id=$id";
